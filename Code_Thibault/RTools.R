...
Comp_train_test = function(train,test){
  ...
  file <- paste(getwd(), "/synthese_TRAIN.xlsx", sep="")
  write.xlsx(synth_train, file)
  
  file <- paste(getwd(), "/synthese_TEST.xlsx", sep="")
  write.xlsx(df, file)
  
  df_type = as.data.frame(table(sapply(train, class)))
  file <- paste(getwd(), "/synthese_type.xlsx", sep="")
  write.xlsx(df_type, file)  
  return(df)
}
